...
// Assuming PatternTypes.ts defines these
...
const PATTERN_FLAG_MAP: Record<string, PatternFlag> = {
  // Example mappings
  'Bullish Engulfing': { isBullish: true },
  'Bearish Engulfing': { isBullish: false },
  // Add Elliott Waves, Gartley, etc.
};
...
export class PatternOverlay {
  ...
  private setupEventListeners() {
    this.eventManager.on('click', (data) => {
      ...
      if (pattern) {
        ...
        // Dispatch to InteractionManager if needed
      }
    });
  }
  ...
  render() {
    ...
    if (scale < 5) return; // Adjust threshold based on TimeScaleEngine
